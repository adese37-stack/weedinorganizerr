(() => {
  ...
  function personalizeGuest() {
    ...
    let i = 0;
    const type = () => {
      el.textContent = guest.slice(0, i++);
      if (i <= guest.length) setTimeout(type, 55);
    };
    type();
  ...
  function setupCalendar() {
    const start = new Date(cfg.wedding.isoDate);
    const end = new Date(cfg.wedding.endIsoDate);
    const fmt = d => d.toISOString().replace(/[-:]/g,"").replace(/\.\d{3}Z$/,"Z");
    const details = encodeURIComponent("Akad dan resepsi pernikahan Puji & Galang.");
    const location = encodeURIComponent(cfg.ceremony.venue + ", " + cfg.ceremony.address);
    const url = `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent("Pernikahan Puji & Galang")}&dates=${fmt(start)}/${fmt(end)}&details=${details}&location=${location}`;
    const btn = $("#calendarButton");
    if (btn) btn.href = url;
  }

  function setupTimelineInteraction() {
    $$(".timeline-item").forEach(item => item.addEventListener("click", () => {
  ...
  function launchConfetti() {
    ...
    resize();
    const colors = ["#a98243","#d8bd82","#f4ecdf","#c9a86a"];
    ...
    const frame=now=>{
      ...
      pieces.forEach(p=>{
        ...
        ctx.fillRect(p.x,p.y,p.r,p.r*.65);
      });
  ...
  addEventListener("DOMContentLoaded",()=>{
    ...
    setupRSVP(); setupGallery(); setupCopy(); setupNavigation(); setupCalendar();
    ...
    openBtn.addEventListener("click",()=>{
      ...
      playMusic();
      setTimeout(()=>{ setupReveal(); setupTimelineInteraction(); },180);
      launchConfetti();
    });
